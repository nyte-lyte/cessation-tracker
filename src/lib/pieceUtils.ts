...
// Used by the analytics page to compute karma/threshold/rankings from the
// live, growing sibling collection (refreshed daily via ISR) rather than the
// frozen genesis snapshot — karma and the liberation threshold are explicitly
// designed to shift as the collection grows.
...
// CBOR metadata, and return the datasets sorted by piece index. Returns null on
// failure (caller falls back to the static healthDataSets).
