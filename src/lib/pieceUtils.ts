...
// ─── Live collection (on-chain sibling discovery) ────────────────────────────
// Ported from main.js cborDecode / lcRefreshSiblings — the on-chain engine
// recomputes minMaxValues, drift, and collection influence from the live sibling
// set as the collection grows. The tracker mirrors that here so its rendering
// matches what collectors see on-chain rather than a frozen 29-piece snapshot.

// Minimal CBOR decoder for the subset of CBOR ord embeds in inscription metadata
// (maps, text strings, unsigned/negative ints, floats — no indefinite-length items).
export function cborDecode(hex: string): unknown {
  ...
  function read(): unknown {
    ...
    if (mt === 7 && ai === 25) {
      const v = (bytes[i] << 8) | bytes[i + 1]; i += 2;
      const e = (v >> 10) & 0x1f, m = v & 0x3ff, s = v >> 15 ? -1 : 1;
      return s * (e === 31 ? (m ? NaN : Infinity) : e === 0 ? m * 5.9604644775e-8 : Math.pow(2, e - 15) * (1 + m / 1024));
    }
    ...
    switch (mt) {
      ...
      case 3: {
        ...
        while (i < e) {
          ...
          else { const b2 = bytes[i++]; s += String.fromCharCode(((c & 0x0f) << 12) | ((b2 & 0x3f) << 6) | (bytes[i++] & 0x3f)); }
        }
        return s;
      }
      case 4: { const arr: unknown[] = []; for (let k = 0; k < n; k++) arr.push(read()); return arr; }
      case 5: { const obj: Record<string, unknown> = {}; for (let k = 0; k < n; k++) { const key = read() as string; obj[key] = read(); } return obj; }
      case 6: return read();
...
// Discover all siblings via /r/children/{engineId} (paginated), decode each one's
// CBOR metadata, and return the datasets sorted by piece index — mirrors
// main.js's lcEffectiveCollection(). Returns null on failure (caller falls back
// to the static healthDataSets).
export async function fetchLiveCollection(engineId: string): Promise<HealthDataSet[] | null> {
  try {
    let page = 0, more = true;
    const fetched: SiblingMeta[] = [];
    while (more) {
      const resp: { ids?: string[]; more?: boolean } = await fetch(`https://ordinals.com/r/children/${engineId}/${page}`).then((r) => r.json());
      for (const id of resp.ids ?? []) {
        try {
          const metaHex = await fetch(`https://ordinals.com/r/metadata/${id}`).then((r) => r.text());
          const trimmed = metaHex.trim().replace(/^"|"$/g, "");
          if (!trimmed) continue;
          const meta = cborDecode(trimmed) as { pieceIndex?: number; dataset?: HealthDataSet };
          if (meta && typeof meta.pieceIndex === "number" && meta.dataset) {
            fetched.push({ pieceIndex: meta.pieceIndex, dataset: meta.dataset });
          }
        } catch { /* skip this sibling */ }
      }
      more = resp.more ?? false;
      page++;
    }
    if (fetched.length === 0) return null;
    return fetched.sort((a, b) => a.pieceIndex - b.pieceIndex).map((s) => s.dataset);
  } catch {
    return null;
  }
}
